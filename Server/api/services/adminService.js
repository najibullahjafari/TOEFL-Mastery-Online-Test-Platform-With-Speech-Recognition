...
const adminService = {
  ...
  async addTest(examObj, response) {
    console.log("Creating test:", examObj);
  ...
  async sendManageTests(examObj, response) {
    ...
    try {
      MongoClient.connect(url, function (err, db) {
        if (err) throw err;
        var dbo = db.db("toefl_mastery");
        console.log("Retrieving tests from exams collection");
        dbo.collection("exams").find().toArray(function (err, result) {
          if (err) throw err;
          console.log("Tests retrieved:", result.length, "tests");
          response.json(result);
          db.close();
        });
      });
